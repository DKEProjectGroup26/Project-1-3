...
public class ReadGraph
{
    ...
    public static void main( String args[] )
    {
        ...
        Integer [][] matrix = new Integer[graph.nodes.size()][graph.nodes.size()];
        ...
        for(int i=0; i<n;i++){
            //matrix[i][i]is the colour that the vertax i+1 can choose and firstly initialize it as 1
            matrix[i][i]=1;
				/*Use this for loop to check the vertax i+1 can't be in the same color with which other vertax(by checking the column i),
            ...
            //This step is to ensure the program go into the "if" only if there's 0 in the rows,since if there's no 0, a will be -1
            if(a>=0){
                if(a<i)
                    matrix[i][i]=a+1;
                    //When there's no other 0 in the lower diagnal of that row, then the color of vertax i+1 should be a new color
                else
                    matrix[i][i]=i+1;
            }else{
